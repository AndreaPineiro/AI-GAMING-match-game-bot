...
headers_vision = {'Ocp-Apim-Subscription-Key': 'fc05d55b05144956ac34e69a4b0974e3'}
...
def calculate_move(gamestate):
    ...
    if analysed_tiles == []:
        ...
        for index in range(num_tiles):
            ...
            analysed_tiles[index]["IndexNoMatch"] = -1 # In case the matching fails, it stores the index of the other card
        
        # Get each category with the back text
        ...
        for index, tile in enumerate(analysed_tiles):
            ...
            #back_dict[gamestate["TileBacks"][index]] = msapi_response['regions'][0]['lines'][0]['words'][0]['text'] + 's'
        #for index, tile in enumerate(analysed_tiles):
            tile['Category'] = msapi_response['regions'][0]['lines'][0]['words'][0]['text'].lower() + 's'
            #print("Back: {}".format(tile['Category']))
    ...
    if gamestate["UpturnedTiles"] != []:
        ...
        # If tried to match was unsuccesfull
        if analysed_all:
            #print("Error")
            if analysed_tiles[gamestate["UpturnedTiles"][0]["Index"]]["IndexNoMatch"] != -1:
                analysed_tiles[gamestate["UpturnedTiles"][0]["Index"]]["State"] = "UNMATCHED"
            else:
                analysed_tiles[gamestate["UpturnedTiles"][0]["Index"]]["IndexNoMatch"] = gamestate["UpturnedTiles"][1]["Index"]
                analysed_tiles[gamestate["UpturnedTiles"][1]["Index"]]["IndexNoMatch"] = gamestate["UpturnedTiles"][0]["Index"]
    ...
    if analysed_all == False:
        unanalysed_tiles = get_unanalysed_tiles()
        if unanalysed_tiles == []:
            analysed_all = True;
        
    if analysed_all:
        match = search_for_matching_tiles(gamestate)
    ...
    # If we don't have any matching tiles
        ...
        # If there are some tiles that we haven't analysed yet
            ...
            # choice?
        #move = [unanalysed_tiles[0], unanalysed_tiles[1]]
        ...
        card2 = len(unanalysed_tiles) - 1 
        while analysed_tiles[unanalysed_tiles[0]]["Category"] == analysed_tiles[unanalysed_tiles[card2]]["Category"] and card2 > 1:
            card2 -= 1 
        move = [unanalysed_tiles[0], unanalysed_tiles[card2]]
...
def analyse_tile(tile, gamestate):
    ...
    #print("  Marked #{}: {}".format(tile["Index"], subject))
            #categ = "Animals"
    #else:
        #categ = "Landmarks"
...
def search_for_matching_tiles(gamestate):
    ...
    for index_1, tile_1 in enumerate(analysed_tiles):
        ...
        for index_2, tile_2 in enumerate(analysed_tiles):
            # Looks for match in the bonus cathegory
            if index_2 > index_1:
                if (tile_1["State"] == tile_2["State"] == "ANALYSED" and tile_1["Category"] == tile_2["Category"] == bonus and tile_1["Subject"] == tile_2["Subject"]  and tile_1["Subject"] is not None and tile_1["IndexNoMatch"] != index_2):
                    # Choose these two tiles
                    # Return the two chosen tiles as a list
                    #print("  Matching Bonus: {}")
                    return [index_1, index_2]
                   
    
    # Tries to guess the bonus
    if gamestate["UpturnedTiles"] == [] and gamestate["Multiplier"] > 60:
        for index_1, tile_1 in enumerate(analysed_tiles):
            for index_2, tile_2 in enumerate(analysed_tiles):
                if index_2 > index_1:
                    if (tile_1["State"] == tile_2["State"] == "ANALYSED" and tile_1["Category"] == tile_2["Category"] == bonus and tile_1["IndexNoMatch"] != index_2):
                        #print("  Guess Bonus: {}")
                        return [index_1, index_2]
                       
        
    # Matches cards we know, even if they are not the bonus
    for index_1, tile_1 in enumerate(analysed_tiles):
        for index_2, tile_2 in enumerate(analysed_tiles):
            if index_2 > index_1:
                if (tile_1["State"] == tile_2["State"] == "ANALYSED" and tile_1["Category"] == tile_2["Category"] and tile_1["Subject"] == tile_2["Subject"] and tile_1["Subject"] is not None and tile_1["IndexNoMatch"] != index_2):
                    #print("  Matching cards Not Bonus: {}")
                    return [index_1, index_2]
    ...
    for index_1, tile_1 in enumerate(analysed_tiles):
        for index_2, tile_2 in enumerate(analysed_tiles):
            if index_2 > index_1:
                if (tile_1["State"] == tile_2["State"] != "MATCHED" and tile_1["Category"] == tile_2["Category"] and tile_1["IndexNoMatch"] != index_2):
                    #print(" Last choice: {}")
                    return [index_1, index_2]
